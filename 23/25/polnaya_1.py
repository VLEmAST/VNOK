...
def load_data_from_files(filepaths):
    ...
    for filepath in filepaths:
        try:
            with open(filepath, "r", encoding="utf-8") as f:
                ...
                loaded_count = 0
                print(f"Начинаем загрузку из файла: {filepath} (всего элементов: {total_items})")

                for item in ijson.items(f, 'item'):
                    if isinstance(item, dict):
                        # Обработка Techniques
                        if "Techniques" in item and isinstance(item["Techniques"], list):
                            for technique in item["Techniques"]:
                                if isinstance(technique, dict) and "Description" in technique and "Name" in technique:
                                    all_descriptions.append(technique["Description"])
                                    all_names.append(technique["Name"])
                        ...
                        loaded_count += 1
                        print(f"Загружено элементов: {loaded_count}/{total_items}", end='\r')

                    else:
                        print(f"Ошибка: Файл '{filepath}' содержит объекты, отличные от словарей.")
                        success = False
                        break
            ...
            print(f"Произошла ошибка при загрузке данных из файла '{filepath}': {e}")
            success = False
            break
    ...
    if success and all_descriptions:
        ...
        for name, desc in zip(all_names, all_descriptions):
            if isinstance(name, str) and isinstance(desc, str):
                preprocessed_names.append(preprocess_text(name, stop_words, morph, punctuation_regex))
                preprocessed_descriptions.append(preprocess_text(desc, stop_words, morph, punctuation_regex))
            else:
                if not isinstance(name, str):
                    print(f"Предупреждение: 'name' не является строкой: {name}")
                if not isinstance(desc, str):
                    print(f"Предупреждение: 'description' не является строкой: {desc}")

        # Создаем DataFrame, если длины списков совпадают
        if len(preprocessed_names) == len(preprocessed_descriptions):
            ...
            print("Ошибка: Длины списков 'preprocessed_names' и 'preprocessed_descriptions' не совпадают.")
            return False
    else:
...
def find_most_similar(
    ...
    try:
        ...
        for i, desc in enumerate(analysis_descriptions):
            # Проверяем, что analysis_names[i] и desc не равны None
            ...
            processed_desc = preprocess_text(
                name + " " + description, stop_words, morph, punctuation_regex
            )
...
def threat_dropdown_changed(e):
    global selected_threat_file_index, loaded_files, threat_file_path
    if threat_dropdown.value is not None: #Добавлена проверка
        selected_threat_file_index = int(threat_dropdown.value)
        threat_file_path = loaded_files[selected_threat_file_index]
        print(f"Выбран файл угроз: {threat_file_path}")
    update_ui()

def vulnerability_dropdown_changed(e):
    global selected_vulnerability_file_index, loaded_files, vulnerability_file_path
    if vulnerability_dropdown.value is not None: #Добавлена проверка
        selected_vulnerability_file_index = int(vulnerability_dropdown.value)
        vulnerability_file_path = loaded_files[selected_vulnerability_file_index]
        print(f"Выбран файл уязвимостей: {vulnerability_file_path}")
    update_ui()
...
def main(page_: ft.Page):
    global page, update_ui, results_text,file_dropdown, results_save_file_picker, model_save_file_picker, general_output, get_data_path, get_stopwords_path, file_dropdown_changed, format_changed, pick_files_data, pick_files_stopwords, show_results_clicked, save_results_button_clicked, save_model_button_clicked, pick_files_model, get_model_path, train_model_threaded, show_snack_bar, results_list, full_file_name_text, selected_format, threat_file_path, vulnerability_file_path, threat_dropdown, vulnerability_dropdown
    ...
    def get_data_path(e: FilePickerResultEvent):
        global is_loading_data, loaded_files, file_dropdown, threat_dropdown, vulnerability_dropdown
        is_loading_data = True
        update_ui()
        ...
        if e.files:
            ...
            threat_dropdown.options.clear()
            vulnerability_dropdown.options.clear()
            ...
            for i, filepath in enumerate(loaded_files):
                ...
                threat_dropdown.options.append(
                    dropdown.Option(key=i, text=truncated_filename)
                )
                vulnerability_dropdown.options.append(
                    dropdown.Option(key=i, text=truncated_filename)
                )
            ...
            if loaded_files:
                ...
                threat_dropdown.value = 0
                vulnerability_dropdown.value = 0
                #full_file_name_text.value = loaded_files[0] #Убрали так как теперь не нужен
                selected_file_index = 0 #Добавили
                selected_threat_file_index = 0 #Добавили
                selected_vulnerability_file_index = 0 #Добавили
                threat_file_path = loaded_files[0]
                vulnerability_file_path = loaded_files[0]
            else:
                file_dropdown.value = None
                threat_dropdown.value = None
                vulnerability_dropdown.value = None
                threat_file_path = None
                vulnerability_file_path = None
    ...
    def compare_threats_vulnerabilities_clicked(e):
        ...
        try:
            ...
            for threat_result in similar_threats:
                ...
                # Добавляем параметры к выводимой информации об угрозе
                threat_info = f"{threat_result}\n" \
                            f"  Нарушение конфиденциальности: {privacy_violation}\n" \
                            f"  Нарушение целостности: {integrity_violation}\n" \
                            f"  Нарушение доступности: {accessibility_violation}"

                results_list.append(threat_info + "\n")  # Добавляем параметры в results_list
                ...
                for vuln, similarity in sorted_vulnerabilities[:3]:
                    ...
                    vuln_description = vuln.get("Description", "Нет описания")
                    vuln_cvss = vuln.get("CVSS2", "")
                    results_list.append(f"  -> Уязвимость: {vuln_name}: \n Описание: {vuln_description} (CVSS2: {vuln_cvss}), Схожесть: {similarity:.4f}\n")
    ...
    #globals()["pick_threat_file"] = pick_threat_file #Удалены
    #globals()["pick_vulnerability_file"] = pick_vulnerability_file #Удалены
    globals()["show_threat_vulnerability_results_clicked"] = compare_threats_vulnerabilities_clicked
    #globals()["get_threat_file_path"] = get_threat_file_path #Удалены
    #globals()["get_vulnerability_file_path"] = get_vulnerability_file_path #Удалены
    globals()["threat_dropdown_changed"] = threat_dropdown_changed
    globals()["vulnerability_dropdown_changed"] = vulnerability_dropdown_changed
    ...
    #threat_file_picker = FilePicker(on_result=get_threat_file_path) #Удалены
    #vulnerability_file_picker = FilePicker(on_result=get_vulnerability_file_path) #Удалены
    ...
    threat_dropdown = Dropdown(
        options=[],
        label="Файл угроз",
        on_change=threat_dropdown_changed,
        value=None,
        width=300,
    )

    vulnerability_dropdown = Dropdown(
        options=[],
        label="Файл уязвимостей",
        on_change=vulnerability_dropdown_changed,
        value=None,
        width=300,
    )
    ...
    #pick_threat_button = ElevatedButton("Выбрать файл угроз", on_click=pick_threat_file) #Удалены
    #pick_vulnerability_button = ElevatedButton("Выбрать файл уязвимостей", on_click=pick_vulnerability_file) #Удалены
    ...
    #page.overlay.append(threat_file_picker) #Удалены
    #page.overlay.append(vulnerability_file_picker) #Удалены
    ...
    page.add(
        ...
        ft.Row([threat_dropdown, vulnerability_dropdown], alignment=ft.MainAxisAlignment.CENTER), # Dropdown для выбора файлов угроз и уязвимостей
